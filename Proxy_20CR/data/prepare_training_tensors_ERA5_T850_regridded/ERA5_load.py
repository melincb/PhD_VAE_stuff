...
def ERA5_quantile_normalise(f, quantiles=None):
    if quantiles is None:
        quantiles = pickle.load(
            open(
                "%s/Proxy_20CR/datasets/ERA5/daily_T2m/regridded_version/Quantiles/total_pctl.pkl"
                % os.getenv("SCRATCH"),
                "rb",
            )
        )
...
def ERA5_load_LS_mask():
    fname = (
        "%s/Proxy_20CR/datasets/ERA5/daily_SST/regridded_version/"
        + "era5_daily_0m_sea_surface_temperature_1979.nc"
    ) % os.getenv("SCRATCH")
    ...
    #print(ddata.data, np.amin(ddata.data), np.amax(ddata.data))
    ddata.data[ddata.data < 0] = 0
...
def ERA5_load_T2m(year, month, day):
    fname = (
        "%s/Proxy_20CR/datasets/ERA5/daily_T2m/regridded_version/"
        + "era5_daily_2m_temperature_%d.nc"
...
def ERA5_load_T2m_climatology(year, month, day):
    if month == 2 and day == 29:
        day = 28
    fname = ("%s/Proxy_20CR/datasets/ERA5/daily_T2m/regridded_version/" + "climatology/%02d/%02d.nc") % (
        os.getenv("SCRATCH"),
        month,
        day,
    )
...
def ERA5_load_T850(year, month, day, ensemble_member=None):
    if not ensemble_member:
        fname = (
            "%s/Proxy_20CR/datasets/ERA5/daily_T850/regridded_version/"
            + "era5_daily_T850_%d.nc"
        ) % (os.getenv("SCRATCH"), year)
        ddata = iris.load_cube(
            fname,
            iris.Constraint(
        ...
        fname = (
            "%s/Proxy_20CR/datasets/ERA5/daily_T850_ensemble_members/regridded_version/"
            + "era5_daily_T850_ensemble_members_%d.nc"#t_%d.nc"
        ) % (os.getenv("SCRATCH"), year)#, ensemble_member)
        if ensemble_member != 'control':
            ens_member_name = f't_{ensemble_member+1}'  # ens. members are 1-9, however labeled as t_2 - t_10 in nc files
        else:
            ens_member_name = 't'   # control forecast
        #print('ens_member_name', ens_member_name)
        ddata = iris.load_cube(
            fname,
            iris.Constraint(
                name=ens_member_name,
                time=lambda cell: cell.point.year == year
                and cell.point.month == month
                and cell.point.day == day
            ),
...
def ERA5_load_T850_climatology(year, month, day):
    if month == 2 and day == 29:
        day = 28
    fname = ("%s/Proxy_20CR/datasets/ERA5/daily_T850/regridded_version/" + "climatology/%02d/%02d.nc") % (
        os.getenv("SCRATCH"),
        month,
        day,
    )
...
def ERA5_load_T850_variability_climatology(year, month, day):
    if month == 2 and day == 29:
        day = 28
    fname = (
        "%s/Proxy_20CR/datasets/ERA5/daily_T850/regridded_version/"
